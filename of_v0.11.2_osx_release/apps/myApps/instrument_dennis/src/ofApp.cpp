...
void ofApp::setup(){
    ...
    // anlayzer
    // patching the scope
    engine.audio_in(0) * dB(gain)   >> engine.blackhole();
   ...
   // engine.audio_in(0) >> dB(12.0f) >> peak1 >> envFollowerPeak >> gateTrigger.in_signal() >> synth.voices[0].in("trig");
    ...
    engine.audio_in(0) * dB(gain) >> synth.voices[0].in("trig");

    engine.audio_in(0) * dB(gain) >> freqToPitch >> synth.voices[0].in("pitch");
    
    synth.ch(0)    * dB(gain)      >> engine.audio_out(1);
    synth.ch(1)    * dB(gain)      >> engine.audio_out(1);
    
    //engine.audio_in(0)  >> absolute >> freqToPitch >> synth.voices[0].in("pitch");
    //float val = 80.0f;
    //val >> synth.voices[0].in("pitch");
    ...
    engine.audio_in(0) >> compressor >> channelGain         >> engine.audio_out(1);
                                        channelGain         >> engine.audio_out(1);
                          compressor >> delaySend >> delayL >> engine.audio_out(1);
                                        delaySend >> delayR >> engine.audio_out(1);
